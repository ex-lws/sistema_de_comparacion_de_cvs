...
class VentanaMenuEditarPerfilCampos(QtWidgets.QMainWindow):
    def __init__(self, parent=None):
        super().__init__(parent)  # Pasamos el parent al constructor
        uic.loadUi('src_ui/menuEditarPerfilCampos.ui', self)
    ...
    def configurar_label_clickeable(self):
        ...
        self.labelActualizarPerfil.mousePressEvent = self.actualizar
    ...
    def actualizar(self, *args):
        ...
        if parametros:
            modificarPerfil(id, parametros)
            QMessageBox.information(self, "Éxito", "Perfil actualizado exitosamente")
            self.close()
        elif parametros is None:
            QMessageBox.warning(self, "Error", "Por favor, complete todos los campos antes de actualizar el perfil.")
    
    
if __name__ == '__main__':
    app = QtWidgets.QApplication(sys.argv)
    ventana = VentanaMenuEditarPerfilCampos()
    sys.exit(app.exec())